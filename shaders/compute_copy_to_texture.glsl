...
layout(binding=0, r32f) uniform writeonly image2DArray tex_volume; //GL_R32F
...
void main(void) {
    ...
    imageStore(tex_volume, ivec3(pos_xy, G_VAL_DEPTH) , vec4(p[id].g_val,0,0,0) );
    imageStore(tex_volume, ivec3(pos_xy, MU_DEPTH) , vec4(p[id].mu,0,0,0) );
    imageStore(tex_volume, ivec3(pos_xy, VAL_MU_DENOISED_DEPTH) , vec4(p[id].val_mu_denoised,0,0,0) );
    imageStore(tex_volume, ivec3(pos_xy, MU_HEAD_DEPTH) , vec4(p[id].mu_head,0,0,0) );
    imageStore(tex_volume, ivec3(pos_xy, P_VEC_DEPTH) , vec4(p[id].p_vec.x,0,0,0) );
    imageStore(tex_volume, ivec3(pos_xy, P_VEC_DEPTH+1) , vec4(p[id].p_vec.y,0,0,0) );
