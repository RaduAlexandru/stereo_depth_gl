...
layout(binding=0, rgba32f) uniform writeonly image2D fbo_rgba_texture; // stores val_mu_denoised mu_head amd p_vec
layout(binding=1, rg32f) uniform writeonly image2D tex_texture; //GL_RG32F //stores the things gval and mu
...
void main(void) {
    ...
    imageStore(tex_texture, pos_xy , vec4(p[id].g_val, p[id].mu ,0,0) );
    imageStore(fbo_rgba_texture, pos_xy, vec4(p[id].val_mu_denoised, p[id].mu_head, p[id].p_vec.x, p[id].p_vec.y) );
